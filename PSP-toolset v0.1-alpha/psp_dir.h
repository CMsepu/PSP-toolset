...
namespace psp_dir{
	//Declare psp standard dir structure
	const std::vector<fs::path> standard_paths = {
		"/PSP/GAME",
		"/PSP/SAVEDATA",
		"/PSP/COMMON",
		"/PSP/SYSTEM",
	...
	bool check_path(const fs::path& p) {
		try{
			...
			fs::path check3 = p / "PSP";
			if(!fs::exists(check1) || !fs::exists(check2) || !fs::exists(check3)) return false;
